---
title: "Patient-hospital analysis"
author: "Harold Achicanoy Estrella"
date: "11/27/2020"
...
The main objective of this analysis consist in identify the factors that contribute to explain why some patients reconsult after the application of some diabetes treatments.

Understanding the causes will help doctors to support decision making on how to address and treat diabetes patients.

## Obtain the data

The first step consists in obtain the data from the web source, unzip the compressed file, and read it using a fast library to load text files.
...
To take a look at the data, the describe command is used:

```{r cars}
psych::describe(tbl)
```

Obtaining that the dataset is conformed by 101766 patients with 50 attributes of information.

## Data pre-processing

The second step consists in develop some pre-processing steps which are:
- Identify and remove variables without variance: due to they do not have information to explain.
- Replace the codes by the right categories to the *admision* variables
- Identify and analyze the variables with more than 25\% of missing data and take the decision if using them or no
- Transform the character variables to factors: just to have consistency of the type of data
- Extract the class mark or central value per age interval to have a numerical proxy variable of the age.
